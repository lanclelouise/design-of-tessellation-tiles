...
class TessellationRendering:
    def add_all_endpoints(self):
        ...
        self.add_endpoint(
            endpoint="/api/canvas",
            endpoint_name="/api/canvas",
            handler=self.intfImgReco,
        )
    ...
    def __init__(self, task_conf, cf_logger):
        ...
        self.rendering_server = task_conf["user_interface"]["web"]["rendering_server"]

        self.conf_render_dash_apps = task_conf["user_interface"]["web"][
            "render_dash_apps"
        ]

        self.conf_update_dash_apps = task_conf["user_interface"]["web"][
            "update_dash_apps"
        ]

        self.conf_dapp_homepage = task_conf["user_interface"]["web"]["conf_homepage"]

        self.plotly_config = task_conf["user_interface"]["web"]["plotly_config"]

        self.conf_case = task_conf["conf_case"]
        assert (
            "case_dir_path" in self.conf_case
        ), 'Key "case_dir_path" is missing in case config.'
        self.case_dir_path = self.conf_case["case_dir_path"]
        assert "conf_model" in self.conf_case, f'Key "conf_model" in not in config'
        ...
        self.conf_model_tile = self.conf_case["conf_model"]["model_tile_reco"]
        self.conf_model_frame = self.conf_case["conf_model"]["model_frame_reco"]
    ...
    def initializeDASH(self):
        ...
        self.app.layout = dbc.Alert("Artful Thinking.")
    ...
    def render_dash_apps(self):
        for conf_dapp in self.conf_render_dash_apps:
            ...
            dapp_layout = html.Div(dapp_layout_list)
    ...
    def intfImgReco(self):
        try:
            ...
            raise Exception("Failed to recognize the image") from e_msg
    ...
    def get_dummy_image(self, height, width, pix_color=255):
        ...
        return array_to_data_url(pix_color * np.ones((height, width, 3), np.uint8))
    ...
    def update_dapp_layout_homepage(self) -> None:
        ...
        conf_style = {"flex-grow": 1, "min-height": "700px"}
        ...
        dapp_layout_list.append(
            dbc.Card(
                [
                    dbc.CardHeader(
                        "Step 1: Draw a tessellation tile image on the canvas below."
                    ),
                    dbc.CardBody(
                        style=conf_style,
                        children=[
        ...
        dapp_layout_list.append(
            dbc.Card(
                [
                    dbc.CardHeader(
                        "Step 2: Tuning the parameters to generate tessellation."
                    ),
                    dbc.CardBody(
                        style=conf_style,
                        children=[
                                    ...
                                    dbc.Card(
                                        [
                                            dbc.CardBody(
                                                children=[
                                                    html.H4(
                                                        "Scale of Tessellation (Amount of Columsn and Rows)",
                                                        className="card-title",
                                                    ),
                                                    dbc.Row(
                                                        children=[
                                    ...
                                    dbc.Card(
                                        [
                                            dbc.CardBody(
                                                children=[
                                                    html.H4(
                                                        "Please fill a number in cell to indicate the adviced image for each tile.",
                                                        className="card-title",
                                                    ),
                                                    DataTable(
                                    ...
                                    dbc.Card(
                                        [
                                            dbc.CardBody(
                                                children=[
                                                    html.H4(
                                                        "Please fill a number in cell to indicate the image direction in each tile.",
                                                        className="card-title",
                                                    ),
                                                    DataTable(
        ...
        info_card = dbc.Card(
            [
                dbc.CardHeader(
                    "A Support System for Artful Design of Tessellations Drawing"
                ),
                dbc.CardBody(
                    [
                        dbc.ListGroup(
                            [
                                dbc.ListGroupItem(info["text"])
                                for info in proc_conf["describe"]["info_list"]
                            ]
                        )
                    ]
                ),
        ...
        self.app.layout = html.Div(
            [
                dbc.Row([dbc.Col(html.Div([info_card]), lg=12)], align="center"),
                html.Br(),
                dbc.Row(
                    [
        ...
        def _update_img_proc(str_data):
            ...
            try:
                ...
                mask_arry = np.where(mask_arry >= 255, 0, 255).astype(np.uint8)
                img_url = array_to_data_url(mask_arry)

                save_img_dp = os.path.join(
                    self.case_dir_path, "bkimg", datetime.now().strftime("%Y-%m-%d")
                )
                os.makedirs(save_img_dp, exist_ok=True)
                self.base64_to_img_buf(img_url).save(
                    os.path.join(
                        save_img_dp,
        ...
        @self.app.callback(
            [
                ...
                Output(
                    component_id=elmid_tess_img_sel,
                ...
                Output(
                    component_id=elmid_tess_agl_sel,
                    component_property="style_data_conditional",
                ),
            ],
            [
        ...
        def _init_tess_param_table(
            ...
            data_tess_img_sel = [
                dict(
                    Rows=f"RowVal_{i+1}",
                    **{
                        f"Col_{j}": get_val(lst_img_sel, i, j)
            ...
            data_tess_agl_sel = [
                dict(
                    Rows=f"RowVal_{i+1}",
                    **{
                        f"Col_{j}": get_val(lst_agl_sel, i, j)
                        for j in range(0, col_cnt)
                    },
                )
                for i in range(0, row_cnt)
        ...
        def _generate_tessellation(
            ...
            http_url = "{}://{}:{}{}".format(
                ...
                self.conf_model_tile["interface"]["host"],
                self.conf_model_tile["interface"]["port"],
                self.conf_model_tile["interface"]["conf_endpoint"]["tessellate"][
                    "endpoint_api"
                ],
            )
            self.logger.info(http_url)
            r = requests.post(http_url, data=tess_post, verify=False)
            rv = r.content
            if type(rv) == dict:
                rv = namedtuple("Struct", rv.keys())(*rv.values())
            tess_rslt = {}
            try:
                ...
                os.makedirs(save_img_dp, exist_ok=True)
                self.base64_to_img_buf(tess_rslt).save(
                    os.path.join(
                        save_img_dp,
